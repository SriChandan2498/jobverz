...
def getSkills(position):
    ...
    for i in skills:
        if(i in trendsDict.keys()):
            ...
            other.add(i)
    skillsDict = {i:trendsDict[i] for i in output}
    finalOutput = sorted(output, key = lambda x : trendsDict[x],reverse=True)
    finalOutput = [[i,trendsDict[i]] for i in finalOutput]
    # step = int(records)
    # result = []
    # i = 1
    # while(i<len(finalOutput)//step):
    #     result = result + [["Skills","Frequency"]] + finalOutput[i*step:(i+1)*step+1]
    ...
    return jsonify({"jobRole":jobRole ,"trendingSKills" : finalOutput, "skillsDict": skillsDict , "otherSkills" : list(other) })
